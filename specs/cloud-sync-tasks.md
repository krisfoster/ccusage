...
| P2-03 | Identity resolution (`userId`, `machineId`, hashed `accounts[]`) per design §4.0, in a new `ccusage-sync` crate. | P0-03, P0-04 | M | 🟡 | Fixture-backed tests incl. missing `~/.claude.json`, multi-account, override precedence. |
...
| P2-06 | Idempotent bucket resolution/creation per §4.1.2, writing `sync.bucket`/`sync.projectId` back to config. Creates with `publicAccessPrevention: enforced` — P5-08 is the only thing allowed to relax it, and only for the dashboard prefix. | P1-06, P2-02, P2-05 | L | 🟡 | Reuse-existing, create-new, 409-retry, 403-explain all tested; refuses to create a second bucket without `--recreate`; a freshly created bucket is verifiably not publicly readable. |
